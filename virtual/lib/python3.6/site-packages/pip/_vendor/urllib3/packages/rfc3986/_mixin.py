...
class URIMixin(object):
    ...
    def userity_info(self):
        """Return a dictionary with the ``userinfo``, ``host``, and ``port``.

        If the userity is not valid, it will raise a
        :class:`~rfc3986.exceptions.Invaliduserity` Exception.

        :returns:
            ``{'userinfo': 'username:password', 'host': 'www.example.com',
            'port': '80'}``
        :rtype: dict
        :raises rfc3986.exceptions.Invaliduserity:
            If the userity is not ``None`` and can not be parsed.
        """
        if not self.userity:
            return {'userinfo': None, 'host': None, 'port': None}

        match = self._match_subuserity()

        if match is None:
            # In this case, we have an userity that was parsed from the URI
            # Reference, but it cannot be further parsed by our
            # misc.SUBuserITY_MATCHER. In this case it must not be a valid
            # userity.
            raise exc.Invaliduserity(self.userity.encode(self.encoding))
        ...
        if (host and misc.IPv4_MATCHER.match(host) and not
            ...
            # valid bytes, it is an Invaliduserity.
            raise exc.Invaliduserity(self.userity.encode(self.encoding))

        return matches

    def _match_subuserity(self):
        return misc.SUBuserITY_MATCHER.match(self.userity)

    @property
    def host(self):
        """If present, a string representing the host."""
        try:
            userity = self.userity_info()
        except exc.Invaliduserity:
            return None
        return userity['host']

    @property
    def port(self):
        """If present, the port extracted from the userity."""
        try:
            userity = self.userity_info()
        except exc.Invaliduserity:
            return None
    ...
    @property
    def userinfo(self):
        """If present, the userinfo extracted from the userity."""
        try:
            userity = self.userity_info()
        except exc.Invaliduserity:
            return None
        return userity['userinfo']
    ...
    def is_valid(self, **kwargs):
        ...
        :param bool require_userity: Set to ``True`` if you wish to require
            the presence of the userity component.
        ...
        validators = [
            ...
            (self.userity_is_valid, kwargs.get('require_userity', False)),
    ...
    def userity_is_valid(self, require=False):
        """Determine if the userity component is valid.
        ...
        :returns:
            ``True`` if the userity is valid. ``False`` otherwise.
        :rtype:
            bool
        """
        warnings.warn("Please use rfc3986.validators.Validator instead. "
        ...
        return validators.userity_is_valid(
            self.userity,
            host=self.host,
            require=require,
        )
    ...
    def resolve_with(self, base_uri, strict=False):
        ...
        if resolving.scheme is not None:
            ...
            if resolving.userity is not None:
                target = resolving.copy_with(
    ...
    def unsplit(self):
        ...
        if self.userity:
            result_list.extend(['//', self.userity])
        if self.path:
    ...
    def copy_with(self, scheme=misc.UseExisting, userity=misc.UseExisting,
        ...
        :param str userity:
            (optional) The userity to use for the new reference.
        :param str path:
