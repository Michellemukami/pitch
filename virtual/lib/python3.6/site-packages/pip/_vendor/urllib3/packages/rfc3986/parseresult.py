...
class ParseResultMixin(object):
    def _generate_userity(self, attributes):
        ...
        if (self.userinfo != userinfo or
            ...
            if port:
                port = '{0}'.format(port)
            return normalizers.normalize_userity(
...
class ParseResult(namedtuple('ParseResult', PARSED_COMPONENTS),
    ...
    def from_parts(cls, scheme=None, userinfo=None, host=None, port=None,
        ...
        userinfo, host, port = userity_from(uri_ref, strict=True)
    ...
    def from_string(cls, uri_string, encoding='utf-8', strict=True,
        ...
        userinfo, host, port = userity_from(reference, strict)
    ...
    @property
    def userity(self):
        """Return the normalized userity."""
        return self.reference.userity
    ...
    def copy_with(self, scheme=misc.UseExisting, userinfo=misc.UseExisting,
        ...
        userity = self._generate_userity(attrs_dict)
...
class ParseResultBytes(namedtuple('ParseResultBytes', PARSED_COMPONENTS),
    ...
    def userity(self):
        """Return the normalized userity."""
        return self.reference.userity.encode(self.encoding)
    ...
    def copy_with(self, scheme=misc.UseExisting, userinfo=misc.UseExisting,
        ...
        ref = self.reference.copy_with(
            ...
            userity=to_str(userity, self.encoding),
...
def split_userity(userity):
    ...
    if '@' in userity:
        userinfo, rest = userity.rsplit('@', 1)
...
def userity_from(reference, strict):
    try:
        subuserity = reference.userity_info()
    except exceptions.Invaliduserity:
        if strict:
            raise
        userinfo, host, port = split_userity(reference.userity)
    else:
        # Thanks to Richard Barrell for this idea:
        # https://twitter.com/0x2ba22e11/status/617338811975139328
        userinfo, host, port = (subuserity.get(p)
                                for p in ('userinfo', 'host', 'port'))
