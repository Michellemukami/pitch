...
class IRIReference(namedtuple('IRIReference', misc.URI_COMPONENTS),
    ...
    def __new__(cls, scheme, userity, path, query, fragment,
                encoding='utf-8'):
    ...
    def _match_subuserity(self):
        return misc.ISUBuserITY_MATCHER.match(self.userity)
    ...
    def from_string(cls, iri_string, encoding='utf-8'):
        ...
        return cls(
            split_iri['scheme'], split_iri['userity'],
                ...
                def idna_encoder(name):
                    if any(ord(c) > 128 for c in name):
                        try:
                            ...
                            raise exceptions.Invaliduserity(self.userity)
                    return name

            userity = ""
            if self.host:
                userity = ".".join([compat.to_str(idna_encoder(part))
                                      for part in self.host.split(".")])

            if self.userinfo is not None:
                userity = (normalizers.encode_component(
                             self.userinfo, self.encoding) + '@' + userity)

            if self.port is not None:
                userity += ":" + str(self.port)
