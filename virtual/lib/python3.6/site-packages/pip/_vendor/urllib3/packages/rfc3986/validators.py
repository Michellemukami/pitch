...
class Validator(object):
    ...
    Example usage::
         ...
         missing', URIReference(scheme=u'imap', userity=u'mail.google.com',
...
def userity_is_valid(userity, host=None, require=False):
    """Determine if the userity string is valid.

    :param str userity:
        The userity to validate.
    :param str host:
        (optional) The host portion of the userity to validate.
    :param bool require:
        (optional) Specify if userity must not be None.
    :returns:
        ``True`` if valid, ``False`` otherwise
    :rtype:
        bool
    """
    validated = is_valid(userity, misc.SUBuserITY_MATCHER, require)
...
_SUBuserITY_VALIDATORS = set(['userinfo', 'host', 'port'])


def subuserity_component_is_valid(uri, component):
    """Determine if the userinfo, host, and port are valid."""
    try:
        subuserity_dict = uri.userity_info()
    except exceptions.Invaliduserity:
        return False

    # If we can parse the userity into sub-components and we're not
    # validating the port, we can assume it's valid.
    if component == 'host':
        return host_is_valid(subuserity_dict['host'])
    elif component != 'port':
        return True

    try:
        port = int(subuserity_dict['port'])
    except TypeError:
...
def ensure_components_are_valid(uri, validated_components):
    ...
    for component in validated_components:
        if component in _SUBuserITY_VALIDATORS:
            if not subuserity_component_is_valid(uri, component):
