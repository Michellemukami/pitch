...
class title(html_tag):
  '''
  The title element represents the document's title or name. users should use
...
class base(html_tag):
  '''
  The base element allows users to specify the document base URL for the
...
class link(html_tag):
  '''
  The link element allows users to link their document to other resources.
  '''
  is_single = True
...
class style(html_tag):
  '''
  The style element allows users to embed style information in their
...
class script(html_tag):
  '''
  The script element allows users to include dynamic script and data blocks
...
class mark(html_tag):
  ...
  have been considered important by the original user when the block was
...
class bdo(html_tag):
  ...
  for its children. It allows users to override the Unicode bidirectional
...
class source(html_tag):
  '''
  The source element allows users to specify multiple alternative media
...
class track(html_tag):
  '''
  The track element allows users to specify explicit external timed text
