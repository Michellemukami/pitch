        ...
        def atom_feed(request):
            ...
            for post in Post.query.limit(10).all():
                feed.add(post.title, post.body, content_type='html',
                         user=post.user, url=post.url, id=post.uid,
...
class AtomFeed(object):
    ...
    :param feed_url: the URL to the feed.  Should be the URL that was
                     requested.
    :param user: the user of the feed.  Must be either a string (the
                   name) or a dict with name (required) and uri or
                   email (both optional).  Can be a list of (may be
                   mixed, too) strings and dicts, too, if there are
                   multiple users. Required if not every entry has an
                   user element.
    ...
    def __init__(self, title=None, entries=None, **kwargs):
        ...
        if not hasattr(self.user, "__iter__") or isinstance(
            self.user, string_types + (dict,)
        ):
            self.user = [self.user]
        for i, user in enumerate(self.user):
            if not isinstance(user, dict):
                self.user[i] = {"name": user}
        ...
        for user in self.user:
            if "name" not in user:
                raise TypeError("user must contain at least a name")
    ...
    def generate(self):
        ...
        # atom demands either an user element in every entry or a global one
        if not self.user:
            if any(not e.user for e in self.entries):
...
class FeedEntry(object):
    ...
    :param user: the user of the entry.  Must be either a string (the
                   name) or a dict with name (required) and uri or
                   email (both optional).  Can be a list of (may be
                   mixed, too) strings and dicts, too, if there are
                   multiple users. Required if the feed does not have an
                   user element.
